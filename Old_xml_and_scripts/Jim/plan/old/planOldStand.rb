...
class PlanOldStand < Plan
  ...
  def replan
    ...
    if EnvironmentModel.beamablePlayMode
      ...
      @queue.clear
      puts "pred"
      @plan << LowSkill.new("stand_front")
      #@plan << LowSkill.new("stand_back")
    end
  end
=begin
      ...
      @plan << GetUp.new(Proc.new{me.on_ground?})
         ...
         #@plan << LowSkill.new("turnleft90")
         #@plan << LowSkill.new("kick_right_faster")
         #@plan << LowSkill.new("fall_right")
      
      
         #@plan << LowSkill.new("kick_right_faster")
         #@plan << CyclicHighSkill.new("walk_fine_fast2_optimized3")
